...
train=contracts[500:len(contracts)]

test=contracts[:500]
for x in train:
    try:

        sol_file_name = os.path.basename(x)#args[1].split('\\')[-1]
        abs_file_name = os.path.abspath(x)
        json_file_name = './train/'+sol_file_name

        #Compile sol -> bin, asm
        print('compile %s' % sol_file_name)
        (if_pragma, contract_name_list),pragma = EVMc.compiler(abs_file_name)
        version = pragma.replace('^', '').replace(' ', '')
        ...
        ordered_ast = EVMf.get_ordered_ast(ast, contract_count, if_pragma,version) # get ordered_ast['Function']
        ...
        for i in range(contract_count):
            if asm[i] != None:
                ...
                block_id, callee, bin, prev_cont, prev_func, prev_block, 0,version)
        ...
        label_file = '../../../Desktop/eth2vec-main/bin/Label.json'
